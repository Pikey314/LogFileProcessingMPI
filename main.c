...
//znajdujemy zawartość plików oraz rozdzielamy je na poszczególne części (dla logów: ip, data, status) za pomocą tokenów
char* date_finder(char* string)
{
	int i = 0;
    char* testString = 0;
    char* token_2;
	char* temporary;
    temporary = malloc(sizeof *temporary * (strlen(string) + 1));
    strcpy(temporary, string);
    token_2 = strtok (temporary," ,-[");
    while (token_2 != NULL){
      if(i++ == 1){
        testString = (char*)malloc(17+1);
        memcpy(testString,token_2,17);
        testString[17] = 0;
        return testString;
      }
      token_2 = strtok (NULL, " ,.-[");
...
char* ip_finder(char* string)
{
    ...
    temporary = malloc(sizeof *temporary * (strlen(string) + 1));
    strcpy(temporary, string);
    token = strtok (temporary,ip_devider);
    while (token != NULL){
      if(i++ == 0){
        return token;
      }
      token = strtok (NULL, ip_devider);
    }
    return NULL;
}

char* status_finder(char* string)
{
	int i = 0;
    char ip_devider[2] = " \0";
    char * token;
...
int main(int argc, char** argv) {
  ...
  char *attr = argc >= 2 ? argv[1] : "addr";
  char *filename = argc >= 3 ? argv[2] : "access.log";
  ...
  if (world_rank == master) {
    ...
    while ((read = getline(&line, &len, f)) != -1) {
      if( strcmp(attr, "addr") == 0 || strcmp(attr, "-addr") == 0 || strcmp(attr, "--addr") == 0 )
      {
        word = ip_finder(line);
      }else if( strcmp(attr, "time") == 0 || strcmp(attr, "--time") == 0 || strcmp(attr, "-time") == 0 ){
        word = date_finder(line);
      }else{
        word = status_finder(line);
      }

      for (word_len = 0; word_len < strlen(word); word_len++) {
        c = word[word_len];
